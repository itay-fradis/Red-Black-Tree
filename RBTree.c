 ...
 * @date 3 June 2020
 * ex3 - RED-BLACK TREE
 * BALANCED BINARY TREE DATA BASE!
***********************************************/
...
//----------- const definitions ---------------
/**
 * @def ALLOC_FAIL_NULL NULL
 * fail memory allocation in functions that return pointer.
 *@def ALLOC_FAIL_ZERO
 * fail memory allocation in functions that return int.
 * @def all other consts:
 * represent failure or success accordingly to the functions they belong
 */
...
#define DATA_WAS_FOUND 1
#define FOR_EACH_FUNC_FAIL 0
#define FOR_EACH_FUNC_SUCCESS 1
#define FALSE 0
#define TRUE 1
...
int forEachRBTreeHelper(Node* node, forEachFunc func, void *args);
...
int RBTreeContains(const RBTree *tree, const void *data)
{
    ...
    if (!findBinaryLeafToDelete(tree->root, tree->compFunc, data)) //do the same as find data in tree
    {
        return DATA_NOT_FOUND;
    }
    return DATA_WAS_FOUND;
}
...
int forEachRBTreeHelper(Node* node, forEachFunc func, void *args)
{
    if (node->left != NULL)
    {
        if (!forEachRBTreeHelper(node->left, func, args))
        {
            return FOR_EACH_FUNC_FAIL;
        }
    }
    ...
    if (node->right != NULL)
    {
        if (!forEachRBTreeHelper(node->right, func, args))
        {
            return FOR_EACH_FUNC_FAIL;
        }
    }
...
void freeRBTreeHelper(RBTree *tree, Node* node)
{
    ...
    freeNode(node, node->parent, tree);
}
